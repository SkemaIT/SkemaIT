...
interface VortexLineOptions {
  count: number;
  color: string;
  glow: number;
}

interface VortexDotOptions {
  count: number;
  size: number;
  color: string;
  glow: number;
...
interface VortexProps {
  ...
  lineOptions?: Partial<VortexLineOptions>;
  dots?: boolean;
  dotOptions?: Partial<VortexDotOptions>;
  comets?: boolean;
  cometOptions?: Partial<VortexCometOptions>;
  repel?: boolean;
  repelOptions?: Partial<VortexRepelOptions>;
...
function createVortex(
  ...
  let strandMat: THREE.LineBasicMaterial | null = null
  ...
  function build() {
    ...
    strandGeo.setAttribute(
      "position",
      new THREE.BufferAttribute(strandPos, 3).setUsage(
        THREE.DynamicDrawUsage
      )
    )
    strandGeo.setAttribute(
      "color",
      new THREE.BufferAttribute(strandCol, 3).setUsage(
        THREE.DynamicDrawUsage
      )
    )
    strandMat = track(
    ...
    if (dotCount > 0) {
      ...
      dotMesh.instanceColor = new THREE.InstancedBufferAttribute(
        dotColors,
        3
      )
    ...
    for (let i = 0; i < cometTotal; i++) {
      ...
      geo.setAttribute(
        "position",
        new THREE.BufferAttribute(trail, 3).setUsage(
          THREE.DynamicDrawUsage
        )
      )
      geo.setAttribute(
        "color",
        new THREE.BufferAttribute(trailCol, 3).setUsage(
          THREE.DynamicDrawUsage
        )
      )
      const lineMat = track(
  ...
  function settle(dt: number) {
    ...
    for (let i = 0; i < dotCount; i++) {
      ...
      for (let k = 0; k < 3; k++) {
        const a =
          -RIPPLE_SPRING * dotShift[at + k] -
          RIPPLE_DAMPING * dotVel[at + k]
        dotVel[at + k] += a * dt
      ...
      const restSq =
        ...
        dotShift[at + 2] ** 2
      const velSq =
        dotVel[at] ** 2 + dotVel[at + 1] ** 2 + dotVel[at + 2] ** 2
      ...
      const sa =
        -SCALE_SPRING * (dotScale[i] - 1) -
        SCALE_DAMPING * dotScaleVel[i]
      dotScaleVel[i] += sa * dt
  ...
  function drawStrand(strand: Strand, now: number, entrance: number) {
    ...
    const alpha =
      Math.min(
        bright *
          0.5 *
          (0.9 + 0.1 * Math.sin(now * 0.18 + strand.pulse)),
        0.7
  ...
  function driveComet(
    ...
    if (comet.boost > 0) {
      ...
      if (comet.boost <= 0) {
        ...
        comet.boostMul =
          1 + (HIT_BOOST - 1) * (comet.boost / HIT_BOOST_TIME)
      }
          ...
          cfg.dotFlicker *
            (0.08 +
              0.92 *
                Math.max(
                  0,
                  Math.sin(t * dot.flickerRate + dot.pulse)
                ) **
                  2.5)
        const swollen =
          dotScale[i] > 1.02 ? 1 + (dotScale[i] - 1) * 0.5 : 1
        const v =
          dot.bright *
          beat *
          cfg.dotGlow *
